...
class Agent:
    ...
    def choose_action(self, observation):
      ...
      probs = dist.log_prob(action).detach().numpy()  #T.squeeze(dist.log_prob(action)).item()
      action = action.numpy() #T.squeeze(action).item()
...
for i in range(n_games):
  ...
  while not done:
    ...
    observation_, reward, done, info = env.step(action.tolist(),step_size, debug=False)
    n_steps +=1 
    score += reward
